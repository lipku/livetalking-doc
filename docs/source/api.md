...
multiavatar_config	|string		|N			|avatar切换配置json字符串，默认值用命令启动参数里的multiavatar_config
...
text				|string		|Y			|文字内容
avatarid			|string		|N			|选择用哪个avatar播报，与multiavatar_config中对应，默认用主avatar('main')
tts			        |string		|N			|配置tts的参考声音和参考文本,默认用命令行参数配置的ref_file和ref_text
